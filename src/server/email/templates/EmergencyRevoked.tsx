...
const NS = 'emails.emergencyRevoked';

export function subject(locale: string): string {
  return t(locale, `${NS}.subject`);
}

export interface EmergencyRevokedEmailProps {
  locale: string;
  grantorEmail: string;
}

export function EmergencyRevokedEmail({ locale, grantorEmail }: EmergencyRevokedEmailProps) {
  return (
