...
/**
 * A failure reported by the interface or the vault, already parsed into frames on
 * the browser side (`src/shared/error-reporting.ts`). The frames are resolved
 * against the source maps sitting next to the bundles in this image, so what
 * leaves is `src/ui/....tsx:73` rather than a column of a minified chunk. The
 * exception is titled by the class name and, in the interface's case, the message;
 * the vault sends none, so its code stands in.
 */
export function captureBrowserError(
  report: { name: string; code: string; message?: string; frames: { filename: string; lineno: number; colno: number; function?: string }[] },
  context: CaptureContext = {}
): void {
  capture(
    {
      browserError: {
        name: report.name,
        message: report.message ?? report.code,
        // Deepest frame last, which is the order the event format expects.
        frames: symbolicateBrowserFrames(report.frames.map((frame) => ({ ...frame, in_app: true })).reverse()),
      },
    },
    { level: 'error', ...context }
  );
