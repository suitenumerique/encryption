...
describe('captureBrowserError', () => {
  const setup = () => {
    resetMonitoring();
    initMonitoring({ dsn: 'https://key@sentry.example/1', environment: 'test' });

    return vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 200 }));
  };
  const eventOf = (fetchMock: ReturnType<typeof setup>) => JSON.parse(((fetchMock.mock.calls[0][1] as RequestInit).body as string).split('\n')[2]);

  it('titles a vault report by class name and code, with the frames deepest-last', async () => {
    ...
    captureBrowserError(
      {
        ...
        frames: [
          { filename: '/vault.js', lineno: 1, colno: 10, function: 'outer' },
          { filename: '/vault.js', lineno: 1, colno: 20, function: 'inner' },
        ],
      },
      { tags: { reportType: 'browser-error' } }
    );
    await flushMonitoring();

    const event = eventOf(fetchMock);
    expect(event.platform).toBe('javascript');
    expect(event.exception.values[0]).toMatchObject({ type: 'VaultError', value: 'VAULT_INTEGRITY_FAILED' });
    expect(event.exception.values[0].stacktrace.frames.map((f: { function: string }) => f.function)).toEqual(['inner', 'outer']);
    expect(event.tags).toEqual({ reportType: 'browser-error' });

    fetchMock.mockRestore();
    resetMonitoring();
  });

  it('titles an interface report by its message, redacted', async () => {
    const fetchMock = setup();

    captureBrowserError({ name: 'TypeError', code: 'UNKNOWN', message: 'failed for alice@example.com', frames: [] });
    await flushMonitoring();

    expect(eventOf(fetchMock).exception.values[0]).toMatchObject({ type: 'TypeError', value: 'failed for [email]' });
