...
function entry(opts: { verified: boolean; key: ArrayBuffer | null; fingerprint: string }) {
  return { verified: opts.verified, encryptionPublicKey: opts.key, identityFingerprint: opts.fingerprint };
}

// TOFU verdict per user, as handleCheckFingerprints would return it.
function checks(map: Record<string, 'trusted' | 'refused' | 'unknown' | 'mismatch'>) {
  return { results: Object.entries(map).map(([userId, status]) => ({ userId, status, knownFingerprint: null, providedFingerprint: '' })) };
}
...
describe('resolveTrustedRecipientKeys (wrap-time trust gate)', () => {
  ...
  it('returns the encryption key for a verified, TOFU-trusted recipient', async () => {
    mockFetch.mockResolvedValue({ users: { bob: entry({ verified: true, key: buf('bobkey'), fingerprint: 'fp-bob' }) } });
  ...
  it("refuses when TOFU says 'mismatch' — a recorded fingerprint that has CHANGED (MITM key swap or unverified rotation)", async () => {
    mockFetch.mockResolvedValue({ users: { bob: entry({ verified: true, key: buf('evilkey'), fingerprint: 'fp-EVIL' }) } });
  ...
  it('refuses when the binding did not verify (encryption key withheld as null)', async () => {
    mockFetch.mockResolvedValue({ users: { bob: entry({ verified: false, key: null, fingerprint: 'fp-bob' }) } });
  ...
  it('wraps for an explicitly-trusted recipient and passes the owner as currentUserId (self-trust)', async () => {
    mockFetch.mockResolvedValue({ users: { bob: entry({ verified: true, key: buf('bobkey'), fingerprint: 'fp-bob' }) } });
    ...
    expect(res.bob).toBeDefined();
    // The owner's own identity is passed as currentUserId so a self-share is trusted.
    // record: true because an actual share records first-sight recipients.
    expect(mockCheck).toHaveBeenCalledWith('alice', { userFingerprints: { bob: 'fp-bob' }, currentUserId: 'alice' }, { record: true });
  });

  it('rejects the whole set if any single recipient is untrusted (all-or-nothing)', async () => {
    mockFetch.mockResolvedValue({
      users: {
        bob: entry({ verified: true, key: buf('bobkey'), fingerprint: 'fp-bob' }),
        eve: entry({ verified: true, key: buf('evekey'), fingerprint: 'fp-eve' }),
      },
    });
  ...
  it('batches: one directory fetch for many recipients', async () => {
    mockFetch.mockResolvedValue({
      users: {
        bob: entry({ verified: true, key: buf('bobkey'), fingerprint: 'fp-bob' }),
        carol: entry({ verified: true, key: buf('carolkey'), fingerprint: 'fp-carol' }),
      },
    });
    ...
    expect(mockFetch).toHaveBeenCalledWith('alice', { userIds: ['bob', 'carol'] });
  });
});
