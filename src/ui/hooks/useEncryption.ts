...
interface UseEncryptionReturn {
  ...
  respondToKeyChallenge: (challengeId: string, ciphertext: string) => Promise<{ response: string }>;
  hasKeys: () => Promise<{ hasKeys: boolean }>;
  getPublicKey: () => Promise<{ publicKey: string }>;
  exportBackup: () => Promise<{ passphrase: string }>;
  importBackup: (passphrase: string) => Promise<{ publicKey: string }>;
  prepareTransferExport: (language?: 'french' | 'english') => Promise<{ encryptedPayload: string; transferPassphrase: string }>;
  claimTransferImport: (encryptedPayload: string, transferPassphrase: string) => Promise<{ publicKey: string }>;
}
...
export function useEncryption({ vaultUrl }: UseEncryptionOptions): UseEncryptionReturn {
  ...
  const generateKeys = useCallback(() => request(MSG_VAULT_GENERATE_KEYS) as Promise<{ publicKey: string }>, [request]);

  const respondToKeyChallenge = useCallback(
    (challengeId: string, ciphertext: string) =>
      request(MSG_VAULT_RESPOND_TO_KEY_CHALLENGE, { challengeId, ciphertext }) as Promise<{ response: string }>,
    [request],
  );

  const hasKeys = useCallback(() => request(MSG_VAULT_HAS_KEYS) as Promise<{ hasKeys: boolean }>, [request]);

  const getPublicKey = useCallback(() => request(MSG_VAULT_GET_PUBLIC_KEY) as Promise<{ publicKey: string }>, [request]);

  const exportBackup = useCallback(() => request(MSG_VAULT_EXPORT_BACKUP) as Promise<{ passphrase: string }>, [request]);

  const importBackup = useCallback(
    (passphrase: string) => request(MSG_VAULT_IMPORT_BACKUP, { passphrase }) as Promise<{ publicKey: string }>,
    [request]
  );
  ...
  const claimTransferImport = useCallback(
    (encryptedPayload: string, transferPassphrase: string) =>
      request(MSG_VAULT_CLAIM_TRANSFER_IMPORT, { encryptedPayload, transferPassphrase }) as Promise<{ publicKey: string }>,
    [request]
  );
