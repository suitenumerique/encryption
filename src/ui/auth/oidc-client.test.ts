...
describe('refreshTokenWithLock', () => {
  ...
  beforeAll(async () => {
    ...
    // time, so the document has to carry one before the import.
    const block = document.createElement('script');

    block.type = 'application/json';
    block.id = RUNTIME_CONFIG_ELEMENT_ID;
    // A served block is validated in full, so the fixture is a whole config even
    // though this suite only exercises the OIDC fields.
