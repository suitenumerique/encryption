...
import { Actions, Icon, LoadingScreen, Screen } from '@encryption/src/ui/components/layout/Screen';
...
function DesignateFlow({ getToken, prefillEmail = null, onBack, onDesignated, markSessionExpired, banner }: DesignateFlowProps) {
  ...
  if (step === 'verify' && contact) {
    return (
      <Screen
        ...
        actions={
          <>
            ...
            <Button variant="tertiary" color="neutral" onClick={onBack} disabled={busy}>
      ...
      >
        ...
        <p className="enc-hint">{t('profile.compare_instruction')}</p>
      </Screen>
    );
  }
    ...
    return (
      ...
      >
        ...
        <div className="enc-section">
          <p className="enc-section__title">{t('emergency.designate_wait_title')}</p>
          ...
          {useCustom && (
            <div>
              <label className="enc-label" htmlFor="emergency-custom-wait">
          ...
          <p className="enc-hint">{t('emergency.designate_wait_hint')}</p>
        </div>

        <div className="enc-section">
          <p className="enc-section__title">{t('emergency.designate_scope_title')}</p>
  ...
  return (
    ...
    >
      <div>
        <label className="enc-label" htmlFor="emergency-contact-email">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          />
          <Button onClick={() => handleSearch()} disabled={busy || email.trim().length === 0} icon={<Icon name="search" />}>
...
function RevealView({ entry, getToken, onDone, markSessionExpired, banner }: RevealViewProps) {
  ...
  const backButton = (
    <Button variant="bordered" color="neutral" onClick={onDone}>
  ...
  if (error) {
    return <Screen title={t('emergency.reveal_title')} banner={<Alert type={VariantType.ERROR}>{error}</Alert>} actions={backButton} />;
  }
        ...
        <Screen
          ...
          description={
            <>
              {pendingRequests.map((req) => (
                <p key={req.id}>
                  {emergencyPhase(req, now) === 'approved' || req.deadline_millis === null ? (
                    <Trans
                      t={t}
                      i18nKey="emergency.prompt_recovery_approved"
              ...
              <p>{t('emergency.prompt_recovery_hint')}</p>
            </>
          }
          actions={
        ...
        >
          {loading && !loadError ? (
            ...
            <>
              ...
              <div className="enc-section">
                <p className="enc-section__title">{t('emergency.trusted_title')}</p>
                <p className="enc-section__subtitle">{t('emergency.trusted_subtitle')}</p>

                {trusted && trusted.length === 0 && <p className="enc-hint">{t('emergency.trusted_empty')}</p>}
                ...
                {(trusted ?? []).map((entry) => {
                  ...
                  return (
                    <div key={entry.id} className={rowAudit === 'tampered' ? 'enc-card enc-card--error' : 'enc-card'}>
                      <div className="enc-card__header">
                        <Identity name={entry.grantee_email} secondary={t('emergency.wait_label', { count: entry.wait_time_days })} />
                        <StatusChip phase={phase} t={t} />
                      </div>

                      {entry.vault_active === false && <p className="enc-hint">{t('emergency.vault_inactive_note')}</p>}
                ...
                <Button variant="secondary" onClick={() => setView('designate')} icon={<Icon name="person_add" />} fullWidth>
              ...
              <div className="enc-section">
                <p className="enc-section__title">{t('emergency.granted_title')}</p>
                <p className="enc-section__subtitle">{t('emergency.granted_subtitle')}</p>

                {granted && granted.length === 0 && <p className="enc-hint">{t('emergency.granted_empty')}</p>}
                ...
                {(granted ?? []).map((entry) => {
                  ...
                  return (
                    <div key={entry.id} className="enc-card">
                      <div className="enc-card__header">
                      ...
                      {phase === 'invited' && (
                        <>
                          <p className="enc-hint">{t('emergency.granted_invited_text', { email: entry.grantor_email })}</p>
                          <Actions layout="row">
                            <Button
                              size="small"
                              variant="bordered"
                      ...
                      {phase === 'requested' && countdown && !countdown.expired && (
                        <>
                          <p className="enc-hint">{t('emergency.granted_requested_text', { time: countdownLabel(countdown, i18n.language) })}</p>
                          <Actions layout="row">
                      ...
                      {phase === 'approved' && (
                        <>
                          <p className="enc-hint">{t('emergency.granted_approved_text', { email: entry.grantor_email })}</p>
                          <Actions layout="row">
                            <Button
                              size="small"
                              disabled={busy}
