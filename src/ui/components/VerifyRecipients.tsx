  ...
  return (
    <Modal isOpen onClose={close} closeOnClickOutside={!busy} size={ModalSize.SMALL} aria-label={t('verify.title')}>
      {phase === 'loading' ? (
        ...
        <Screen
          title={t('verify.title')}
          description={recipients.length > 1 ? t('verify.explanation_many') : undefined}
          banner={
            <>
              {banner}
        ...
        >
          {recipients.map((recipient) => {
            ...
            return (
              <div key={recipient.userId} className="enc-screen__body">
                {!recipient.trusted && (
                  <Alert type={recipient.status === 'mismatch' ? VariantType.WARNING : VariantType.ERROR}>
