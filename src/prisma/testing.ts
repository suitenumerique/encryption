...
async function start(): Promise<void> {
  ...
  const tables = await client.$queryRaw<{ tablename: string }[]>`SELECT tablename FROM pg_tables WHERE schemaname = current_schema()`;
  truncateStatement = `TRUNCATE TABLE ${tables.map((row) => `"${row.tablename}"`).join(', ')} CASCADE`;
}
