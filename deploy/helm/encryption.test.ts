...
interface Manifest {
  ...
  spec?: { template?: { spec: { serviceAccountName?: string; containers: { env: EnvEntry[] }[] } } };
}
...
describe('the chart', () => {
  ...
  // The failure this guards against only shows on a real cluster: Helm runs pre-install
  // hooks before it creates the release's objects and pre-upgrade hooks before it updates
  // them. A migration Job reading the release's own ServiceAccount never gets a pod on a
  // first install ("serviceaccount not found"), and one reading the release's Secret sees
  // the previous revision's content on an upgrade. So whatever the Job reads must be a
  // hook of both phases that runs before it, or something the chart does not render.
  it('only gives the migration Job objects that exist before it runs', () => {
    for (const { docs } of [full, minimal]) {
      const job = docs.find((document) => document.kind === 'Job');

      if (!job?.spec?.template) throw new Error('no migration Job in the render');

      const weight = (document: Manifest) => Number(document.metadata.annotations?.['helm.sh/hook-weight'] ?? 0);
      const runsBeforeJob = (document: Manifest) => {
        const phases = document.metadata.annotations?.['helm.sh/hook']?.split(',') ?? [];

        return phases.includes('pre-install') && phases.includes('pre-upgrade') && weight(document) < weight(job);
      };
      const rendered = (kind: string, name: string) => docs.find((document) => document.kind === kind && document.metadata.name === name);

      const accountName = job.spec.template.spec.serviceAccountName ?? 'default';
      const account = rendered('ServiceAccount', accountName);

      expect(account === undefined || runsBeforeJob(account), `ServiceAccount ${accountName} must be a hook running before the Job`).toBe(true);
      expect(accountName, 'the migrator must not share the server account').not.toBe(
        docs.find((document) => document.kind === 'Deployment')?.spec?.template?.spec.serviceAccountName
      );

      for (const entry of job.spec.template.spec.containers[0].env) {
        const secretName = entry.valueFrom?.secretKeyRef?.name;

        if (!secretName) continue;

        const secret = rendered('Secret', secretName);

        expect(secret === undefined || runsBeforeJob(secret), `Secret ${secretName} (${entry.name}) must be a hook running before the Job`).toBe(
          true
        );
      }
    }
  ...
  it('requires on tokens the client the interface signs in with', () => {
    for (const env of [serverEnv(full.docs), serverEnv(minimal.docs)]) {
      // The server checks `azp` against OIDC_SERVER_CLIENT_ID, and the interface's tokens
      // are issued to OIDC_CLIENT_ID: any difference rejects every call it makes.
      expect(env.get('OIDC_SERVER_CLIENT_ID')?.value).toBe(env.get('OIDC_CLIENT_ID')?.value);
    }
  });
  ...
  it('renders nothing for what is off', () => {
    ...
    expect(
      docs.filter((document) => document.metadata.labels?.['app.kubernetes.io/component'] === 'migration').map((document) => document.kind),
      'no migration hook object without the migration'
    ).toEqual([]);
