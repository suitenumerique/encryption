...
Whichever way the product images are produced, the stack only ever sees a reference: `ci/staging-values.yaml` (or the `images` inputs of the preview helmfile) is where it is set, and the coherence checks do not look at it. The encryption image of a pull request is published by this repository's own preview workflow (see "One environment per pull request"); for Docs and Drive, two ways exist:

1. **The products' own preview publishing.** A pull request in the upstream Docs repository carrying the `preview` label gets its images pushed by the Docs CI as `lasuite/impress-backend:pr-<number>`, `impress-frontend:pr-<number>` and `impress-y-provider:pr-<number>`, republished on every push to that pull request. This is the path with no manual step: open the integration branches as upstream pull requests, label them, and point the stack at the `pr-<number>` tags (for a preview, with a `docs:pr-<number>` label on the encryption pull request, see below). Drive does not push images for pull requests today; once it does, the same applies to it.
...
The same helmfile with `feature` set to the pull request number is the preview environment: the chart is read from the checkout, so a pull request that changes it is previewed with its own chart, and nothing is published for it. The machinery lives in the deployment repository, not here: `deploy/argocd/applicationset-preview.example.yaml` is an ApplicationSet whose pull request generator lists the pull requests of this repository carrying the `preview` label and renders the helmfile at each one's commit through a helmfile plugin; when the pull request closes or loses the label, the Application goes and prunes its namespace. The inputs travel as environment variables of the Application, `FEATURE` (the number) and `DOMAIN`, because that is what a plugin can pass to the command it runs: the helmfile reads them (also under the `ARGOCD_ENV_` prefix a sidecar plugin adds) when no state value is given. Two more, `DOCS_TAG` and `DRIVE_TAG`, name the product images under test, one tag per product for all its images, and the ApplicationSet reads them from labels of the pull request, `docs:pr-2694` or `drive:encryption`: the committed defaults stay `main`, the test versions live on the pull request, and there is nothing to undo before merging. An Ingress carrying `ci-docs.example.org`, the defaults, means the variables did not reach the render. This repository's part is `.github/workflows/preview.yml`: the encryption image of the pull request's own commit, pushed to Docker Hub as `lasuite/encryption:pr-<number>` on every push, which the helmfile derives from the number by itself (and `sha-<8 chars>`, for an ApplicationSet that passes `ENCRYPTION_IMAGE` to get a rollout on every push rather than a restart); a comment with the URLs; and a webhook nudge so Argo CD re-evaluates now rather than at its next poll. The same workflow run by hand ("Run workflow" on a `feat/<name>` branch, the prefix saying the build is a feature in progress; any other branch is refused with a message) publishes that branch under its name, `feat-<name>`, for a stack to point at without a pull request. Releases and `main` are the CI's alone, and the workflow cleans after itself: a pull request's tag goes when it closes or loses the label, the other preview tags after thirty days. It pushes and deletes with the Docker Hub token of the `release` environment, the CI's, whose deployment branch rules must admit a pull request's merge ref (`refs/pull/*/merge`) and the feature branches (`feat/*`); a token of its own in an environment of its own would keep the release credential with the release job alone, and is a two-line change when one exists.
