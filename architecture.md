...
**The operator side.** `src/maintenance/` is the tooling, run from a checkout with `npm run maintenance`. `keygen` writes the key file (the secret key never leaves that machine) and prints the public key to configure; `check`, `decrypt` and `rewrap` read rows exported from a product table on stdin (one JSON object per line or a JSON array, with `maintenanceKey`, optionally `encryptedContent` and `recipients`) and write one JSON result per line; `fetch-public-keys` resolves subs through the public directory with the same binding verification the vault runs. `rewrap` covers the two shapes of repair: wrap the same resource key again for recipients who lost a usable copy, or, with `--rotate-key`, mint a new resource key and re-encrypt the content. `src/maintenance/escrow.test.ts` proves the whole loop against the real vault operations: create, escrow, decrypt as the operator, re-wrap for a new holder, rotate, and read back from that holder's device.
