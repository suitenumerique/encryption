...
## Appendix C: Maintenance escrow (a deployment option)

A deployment can keep, for every encrypted resource, one extra wrapped copy of the resource key for an **operator-held maintenance key**. It is a deployment option, off unless `MAINTENANCE_ESCROW_PUBLIC_KEY` is set, and each operator of this open-source service decides for themselves whether to run it and for how long. It is worth running when the operator is not yet confident in the deployment (a defect in the client, a format change, or a corrupted wrapped key then stays repairable **on the stored data**, without shipping a client patch and waiting for every user to open every document from an enrolled device), and when the operator wants a guaranteed way out later, such as a migration of the stored content to another tool or another encryption scheme. The escrow lets the operator read and re-encode a resource offline, then write the result back; users never take part.

**The trade-off, stated plainly.** With the escrow, the content is readable by whoever holds the maintenance secret key, so end-to-end encryption is lowered by exactly that one holder. This is largely acceptable when the key is kept the way the custody section below describes, because the property that matters most is preserved: a leak of the product database, of its backups, or of the encryption server yields ciphertext and wrapped copies that nobody on the network side can open, since the secret key exists only offline. What the escrow gives up is the guarantee that the operator themselves cannot read; what it keeps is the protection against every party that is not the custodian of that key.

**What is escrowed, and what is not.** The copy is the resource's symmetric key (a document's key in Docs, the encryption root's key in Drive), wrapped under the maintenance public key with the same X-Wing wrap every recipient gets (`encryptSymmetricKeyForUsers`, one more entry). It is never a user's private key, identity key, or VRK: the maintenance key can open and re-encode content, and nothing else. It cannot sign, register, share as a user, or impersonate anyone. A resource stays end-to-end encrypted in the sense that matters, since only holders of a wrapped copy can read it; the escrow adds one holder with a defined custody, and the operator's ability to read is what makes the maintenance possible.

**Where it is produced and where it lives.** The vault produces it, in `encrypt-without-key` and `share-keys`, **after** the recipient trust gate has run (Section 3.4): the maintenance key is read from the vault's runtime configuration, at the same trust level as the allowed origins, never from the request. It comes back in a field of its own, `maintenanceKey`, beside `encryptedKeys`, so a product cannot make the vault wrap for another key, cannot remove it, and never sees it in a recipient list. The product persists it verbatim on the resource, next to the ciphertext (a nullable column on the document in Docs, on the encryption root in Drive). Nothing about a document ever reaches the encryption server: the copy sits where the ciphertext sits, which is also where the maintenance happens. In Drive, one copy per encryption root is enough, because every nested item's key is reachable from the root key through the symmetric chain the product already stores; the nested operations therefore produce no copy.

**Coverage.** Every resource created or shared while the key is configured carries a current copy. A share refreshes it, and a share with an empty recipient list returns only the copy, which is how a product backfills a resource created before the option was enabled, lazily, the next time any holder opens it. `npm run maintenance -- check` audits an export for rows whose copy does not open.

**The operator side.** `src/maintenance/` is the tooling, run from a checkout with `npm run maintenance`. `keygen` writes the key file (the secret key never leaves that machine) and prints the public key to configure; `check`, `decrypt` and `rewrap` read rows exported from a product table, from a file (`--in`, streamed line by line so the size of the export does not matter) or from stdin (one JSON object per line or a JSON array, with `maintenanceKey`, optionally `encryptedContent` and `recipients`), and write one JSON result per line to a file (`--out`, owner-readable only) or to stdout; `fetch-public-keys` resolves subs through the public directory with the same binding verification the vault runs. `rewrap` covers the two shapes of repair: wrap the same resource key again for recipients who lost a usable copy, or, with `--rotate-key`, mint a new resource key and re-encrypt the content. `src/maintenance/escrow.test.ts` proves the whole loop on the real parts: a product table in PostgreSQL (binary columns, exported and written back in SQL), the public directory route reached through the generated API client with genuinely signed registrations, and the vault operations as the users' devices. It covers create, escrow, audit and decrypt as the operator, repair of corrupted wrapped copies, a resource key rotation combined with a maintenance key rotation, the backfill, and the refusal to wrap for a directory record altered in the database.

**Custody and lifecycle.** The secret key is the single most valuable secret of the deployment and is treated accordingly: generated offline with `keygen` on an isolated machine, never copied to a server, a CI system, or a shared drive. The recommended custody is a **physical vault under the responsibility of the security officer (RSSI / CISO)**, and better still a **Shamir split** of the key file across several people (the security officer, the data protection officer, a service owner) so that no single person can read content alone and a maintenance operation is a deliberate, witnessed act. The key is used only from an isolated machine, for the duration of the operation. When the operator no longer wants the option, unsetting the variable stops new copies, dropping the column removes the existing ones, and destroying the secret key is what actually closes the window, since a copy that still exists stays decryptable by whoever holds the key. A maintenance key rotation is a `rewrap --next-public-key` over the exported rows, and is the same operation as any repair.
