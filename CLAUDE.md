...
- `src/maintenance/` — operator CLI (`npm run maintenance`, commander) for the maintenance escrow: keygen, check, decrypt, rewrap, fetch-public-keys over JSON rows, from `--in`/`--out` files (streamed) or stdin/stdout
